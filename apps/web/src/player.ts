...
async function start() {
  ...
  try {
    ...
    ) as { scene: unknown; fonts: Record<string, string>; licenses: string };
    ...
    document.getElementById("mode")!.textContent =
      scene.pointer.mode === "recorded"
        ? "Recorded pointer"
        : scene.pointer.mode === "fixed"
          ? "Fixed pointer"
          : "Pointer inactive";
    let time = 0;
    ...
    live.onclick = () => {
      ...
      live.setAttribute("aria-pressed", String(liveEnabled));
      live.textContent = liveEnabled ? "Live pointer: on" : "Live pointer: off";
    };
    canvas.onpointermove = (event) => {
      const rect = canvas.getBoundingClientRect();
      pointer = {
        x: ((event.clientX - rect.left) * scene.artboard.width) / rect.width,
        y: ((event.clientY - rect.top) * scene.artboard.height) / rect.height,
        presence: 1,
      };
    };
    ...
    function draw(now: number) {
      if (playing && !document.hidden)
        time = (time + Math.max(0, now - last)) % scene.timeline.durationMs;
      last = now;
      paintFrame(
        ctx!,
        evaluateScene(compiled, {
          timeMs: time,
          pointer: liveEnabled ? pointer : samplePointer(scene, time),
        }),
        1,
      );
      slider.value = String(time);
      clock.textContent = `${(time / 1000).toFixed(1)} s`;
      requestAnimationFrame(draw);
    }
    status.textContent = "";
    // Exposed for reproducible export verification, with no editable scene or network capability.
    Object.defineProperty(window, "livingPosterPlayer", {
      value: {
        seek(ms: number) {
          playing = false;
          time = Math.max(0, Math.min(ms, scene.timeline.durationMs));
          updateToggle();
          paintFrame(
            ctx!,
            evaluateScene(compiled, {
              timeMs: time,
              pointer: samplePointer(scene, time),
            }),
            1,
          );
        },
