...
export async function recentDrafts(): Promise<ArchivedDraft[]> {
  ...
  return rows
    .filter((row: any) => row?.updatedAt && row?.documentId)
...
export function archiveDraft(draft: Recovery): Promise<void> {
  ...
  const next = chain
    ...
    .then(async () => {
      const d = await db();
      await d.put("workspace", copy, `draft:${draft.documentId}`);
    });
  chain = next;
  return next;
}
export function updateArchivedHead(
  documentId: string,
  projectId: string,
  serverHead: string,
): Promise<void> {
  const next = chain
    ...
    .then(async () => {
      const d = await db();
      const archived = await d.get("workspace", `draft:${documentId}`);
      if (archived)
        await d.put(
          "workspace",
          { ...archived, projectId, serverHead },
          `draft:${documentId}`,
        );
    });
  chain = next;
...
export function persistDraft(draft: Recovery): Promise<void> {
  ...
  const next = chain
    ...
    .then(async () => {
      ...
      await tx.store.put(copy, "current");
      await tx.store.put(
        { ...copy, updatedAt: Date.now() },
        `draft:${copy.documentId}`,
      );
      await tx.done;
    });
  chain = next;
