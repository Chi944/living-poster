...
import {EXAMPLES,cloneScene,reviseScene,validateScene,newId,applyOperations,type Scene,type EditOperation,type Layer} from '../../../packages/core/src';
import {api,post,ApiError,type Project} from './api';
import {persistDraft,recoverDraft,archiveDraft,updateArchivedHead,type OutboxItem} from './drafts';
export type AiCapture={revisionId:string;generation:number;epoch:number;documentId:string;};
type EditorState={
  scene:Scene; selected:string[]; past:Scene[]; future:Scene[]; gesture:Scene|null;
  documentId:string;projectId:string|null;serverHead:string|null;name:string;
  mutationEpoch:number;requestGeneration:number;affected:string[];notice:string;
...
export function setGeometryValidation(validator:((scene:Scene)=>void)|null){validateGeometry=validator;}
export const useEditor=create<EditorState>((set,get)=>({
 scene:initial,selected:[],past:[],future:[],gesture:null,documentId:newId(),projectId:null,serverHead:null,name:EXAMPLES[0].title,
 mutationEpoch:0,requestGeneration:0,affected:[],notice:'',playing:!matchMedia('(prefers-reduced-motion: reduce)').matches,timeMs:0,livePointer:false,recording:false,
 outbox:[],saveState:'Saved on this device',recovered:false,
 commit(change,label='Updated poster'){
  const state=get();try{const next=cloneScene(state.scene);change(next);const revised=reviseScene(next);validateScene(revised);validateGeometry?.(revised);
 ...
 beginGesture(){set(state=>({gesture:cloneScene(state.scene),playing:false,mutationEpoch:state.mutationEpoch+1,affected:[]}));},
 moveGesture(dx,dy){const state=get();if(!state.gesture)return;const next=cloneScene(state.gesture);next.layers.forEach(layer=>{if(state.selected.includes(layer.id)&&!layer.locked){layer.layout.x=Math.max(16,Math.min(1064,layer.layout.x+dx));layer.layout.y=Math.max(16,Math.min(1334,layer.layout.y+dy));}});set({scene:next});},
 endGesture(cancel=false){const state=get();if(!state.gesture)return;const before=state.gesture;if(cancel||JSON.stringify(before)===JSON.stringify(state.scene)){set({scene:before,gesture:null});return;}try{validateScene(state.scene);validateGeometry?.(state.scene);set({scene:reviseScene(state.scene),past:[...state.past.slice(-79),before],future:[],gesture:null,notice:'Moved selection'});}catch(error){set({scene:before,gesture:null,notice:trimError(error)});}},
 removeSelected(){const selected=get().selected;get().commit(scene=>{scene.layers=scene.layers.filter(layer=>!selected.includes(layer.id)||layer.locked);const alive=new Set(scene.layers.map(l=>l.id));scene.layers.forEach(layer=>{layer.behaviors=layer.behaviors.filter(b=>!('anchor' in b.params)||b.params.anchor.type!=='layer'||alive.has(b.params.anchor.layerId));});},'Deleted selection');set({selected:[]});},
 reorder(id,direction){get().commit(scene=>{const index=scene.layers.findIndex(l=>l.id===id);const next=Math.max(0,Math.min(scene.layers.length-1,index+direction));const [layer]=scene.layers.splice(index,1);scene.layers.splice(next,0,layer);},'Reordered layer');},
 beginAi(){const state=get();const generation=state.requestGeneration+1;set({requestGeneration:generation});return {revisionId:state.scene.revision.id,generation,epoch:state.mutationEpoch,documentId:state.documentId};},
...
export function updateLayer(id:string,change:(layer:Layer)=>void,label?:string){return useEditor.getState().commit(scene=>{const layer=scene.layers.find(l=>l.id===id);if(layer)change(layer);},label);}
export function persistCurrent(){const s=useEditor.getState();return persistDraft({scene:s.gesture??s.scene,documentId:s.documentId,projectId:s.projectId,serverHead:s.serverHead,name:s.name,outbox:s.outbox});}
let syncing=false;
...
export async function initializeRecovery(){try{const saved=await recoverDraft();if(saved){validateScene(saved.scene);useEditor.setState({...saved,past:[],future:[],selected:[],mutationEpoch:1,requestGeneration:1,recovered:true,notice:'Recovered your latest draft from this device.'});}}catch(error){useEditor.setState({notice:`Device recovery unavailable: ${trimError(error)}`});}
