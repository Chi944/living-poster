...
const scene = validateScene({
  ...
  id: "dev-probe",
  revision: { id: "dev-r1", parentId: null },
  seed: 51,
  artboard: { width: 1080, height: 1350, background: "#fafafa" },
  ...
  layers: [
    {
      ...
      locked: false,
      opacity: 1,
      layout: { x: 120, y: 310, rotationDeg: 0 },
      behaviors: [],
    },
    {
      id: "details",
      ...
      locked: false,
      opacity: 1,
      layout: { x: 120, y: 690, rotationDeg: 0 },
      behaviors: [],
    },
    {
      id: "badge",
      ...
      locked: false,
      opacity: 1,
      layout: { x: 760, y: 800, rotationDeg: 0 },
      behaviors: [],
    },
  ],
});
...
const provider = createOllamaProvider(
  ...
  process.env.LP_PROBE_MODEL ?? "qwen3:4b",
);
if (process.env.LP_PROBE_SET === "ambiguity" && scene.layers[1].kind === "text")
  scene.layers[1].text = "OPEN STUDIO";
const probes: [string, string[]][] =
  process.env.LP_PROBE_SET === "ambiguity"
    ? [
    ...
    : [
        ...
        [
          "Move the selected elements eighteen pixels to the left.",
          ["lead", "badge"],
        ],
...
for (const [instruction, selectedLayerIds] of probes) {
  ...
  try {
    const result = await provider.generate(input, AbortSignal.timeout(120000));
    console.log(
      JSON.stringify({ instruction, ms: Date.now() - started, ...result }),
    );
  } catch (e) {
    console.log(
      JSON.stringify({
