...
export interface Frame {
  ...
  baseBounds: Record<string, Bounds>;
...
function browserMeasure(
  ...
  const key = `${fontId}|${fontSize}|${text}`;
  const cached = metricCache.get(key);
  if (cached) return cached;
  if (!measurementContext) {
    if (typeof document === "undefined")
      throw new Error("Text layout requires a loaded browser font");
    measurementContext = document.createElement("canvas").getContext("2d");
  }
  if (!measurementContext) throw new Error("Canvas 2D is unavailable");
  const c = measurementContext;
  c.font = fontCss(fontId, fontSize);
  c.fontKerning = "none";
  c.textAlign = "left";
  ...
  if (Object.values(result).some((v) => !Number.isFinite(v)))
    throw new Error(`Font metrics failed for ${fontId}`);
  if (metricCache.size > 8192) metricCache.clear();
...
function unitBounds(
  ...
  const d = dimensions(width, height, angle);
  return {
    x: x - d.width / 2,
    y: y - d.height / 2,
    width: d.width,
...
function baseBounds(l: Layer, u: BaseUnit): Bounds {
  const p = rotate(u.x, u.y, l.layout.rotationDeg * DEG);
  return unitBounds(
    l.layout.x + p.x,
    l.layout.y + p.y,
    u.width,
...
function layoutText(l: TextLayer, measure: GlyphMeasurer): BaseUnit[] {
  ...
  for (const [lineIndex, line] of l.text.split("\n").entries()) {
    const chars = graphemes(line),
      metrics = chars.map((g) => measure(l.fontId, l.fontSize, g));
    const advance =
      metrics.reduce((sum, m) => sum + m.width, 0) +
      Math.max(0, chars.length - 1) * l.fontSize * l.trackingEm;
    let cursor =
      l.align === "center" ? -advance / 2 : l.align === "right" ? -advance : 0;
    for (let i = 0; i < chars.length; i++, index++) {
      const m = metrics[i]!,
        text = chars[i]!;
      if (text.trim() && m.right + m.left > 0 && m.ascent + m.descent > 0) {
        const cx = (m.right - m.left) / 2,
          cy = (m.descent - m.ascent) / 2;
        units.push({
          kind: "glyph",
          layerId: l.id,
          index,
          x: cursor + cx,
          y: lineIndex * l.fontSize * l.lineHeight + cy,
          width: m.left + m.right,
          height: m.ascent + m.descent,
          text,
          font: fontCss(l.fontId, l.fontSize),
          offsetX: -cx,
          offsetY: -cy,
          cornerRadius: 0,
        });
...
export function compileScene(
  input: Scene,
  measure: GlyphMeasurer = browserMeasure,
): CompiledScene {
  const scene = validateScene(input),
    pivots: CompiledScene["pivots"] = Object.create(null);
  let glyphCount = 0,
    behaviorCount = 0;
  const layers = scene.layers.map((layer) => {
    pivots[layer.id] = { x: layer.layout.x, y: layer.layout.y };
    behaviorCount += layer.behaviors.length;
    if (layer.kind === "text" && measure === browserMeasure)
      assertFontReady(layer.fontId);
    const units: BaseUnit[] =
      layer.kind === "text"
        ? layoutText(layer, measure)
        : [
            {
              kind: layer.shape,
              layerId: layer.id,
    ...
    for (const unit of units) {
      const bounds = baseBounds(layer, unit);
...
function scatterVector(
  ...
  const key = `${layerId}\0${behaviorId}\0${index}`;
  ...
  return {
    x: Math.cos(theta) * r,
    y: Math.sin(theta) * r,
    angle: (random() * 2 - 1) * angle,
  };
}
function phase(b: Behavior, t: number): number | null {
...
export function evaluateScene(
  ...
  if (
    ...
    (input.pointer &&
      Object.values(input.pointer).some((v) => !Number.isFinite(v)))
  )
    throw new Error("Evaluation requires finite time and pointer coordinates");
  ...
  const frame: Frame = {
    ...
    baseBounds: Object.create(null),
    boundsCorrections: 0,
    glyphCount: compiled.glyphCount,
    behaviorCount: compiled.behaviorCount,
  };
  for (const { layer: l, units, baseBounds: base } of compiled.layers) {
    ...
    for (const type of ORDER) {
      ...
      switch (b.type) {
        case "float": {
          const a = 2 * Math.PI * b.params.cycles * u + b.params.phase;
          dx += e * b.params.amplitudeX * Math.sin(a);
          dy += e * b.params.amplitudeY * Math.cos(a);
          commonAngle += e * b.params.rotationAmplitudeDeg * Math.sin(a);
          break;
        }
        case "orbit": {
          const a = b.params.anchor,
            point = a.type === "point" ? a : compiled.pivots[a.layerId]!,
            x = l.layout.x - point.x,
            y = l.layout.y - point.y,
            rot = rotate(
              x,
              y,
              2 * Math.PI * b.params.direction * b.params.cycles * smooth(u),
            );
          dx += rot.x - x;
          dy += rot.y - y;
          break;
        ...
        case "scatter": {
          if (b.scope === "glyph") break;
          const v = scatterVector(
              scene.seed,
              l.id,
              b.id,
              -1,
              b.params.radius,
              b.params.rotationMaxDeg,
            ),
            h = scatterEnvelope(u, b.params.outEnd, b.params.returnStart);
          dx += v.x * h;
          dy += v.y * h;
          commonAngle += v.angle * h;
          break;
        }
        case "attract": {
          const a = b.params.anchor,
            ...
            v = limit(
              point.x - l.layout.x,
              point.y - l.layout.y,
              b.params.maxDistance,
            );
          dx += v.x * e * b.params.strength;
          dy += v.y * e * b.params.strength;
          break;
        }
        case "repel": {
          const p = input.pointer;
          if (!p) break;
          const x = l.layout.x - p.x,
            y = l.layout.y - p.y,
            r = Math.hypot(x, y),
            f =
              (e *
                clamp(p.presence, 0, 1) *
                b.params.maxDistance *
                Math.max(0, 1 - r / b.params.radius) ** 2) /
              Math.sqrt(r * r + 256);
          dx += x * f;
          dy += y * f;
          break;
    ...
    for (const unit of units) {
      ...
      for (const b of l.behaviors) {
        ...
        if (b.type === "wave")
          gy +=
            ...
            b.params.amplitude *
            Math.sin(
              2 * Math.PI * b.params.cycles * u -
                (2 * Math.PI * unit.index) / b.params.wavelength +
                b.params.phase,
            );
        if (b.type === "scatter") {
          const v = scatterVector(
      ...
      frame.bounds[l.id] = union(frame.bounds[l.id] ?? null, box);
    }
  }
  return frame;
}
export function paintFrame(
  ...
  for (const unit of frame.units) {
    ...
    if (unit.kind === "glyph") {
      ctx.font = unit.font!;
      ctx.fillText(unit.text!, unit.offsetX, unit.offsetY);
    } else {
      ctx.beginPath();
      if (unit.kind === "ellipse")
        ctx.ellipse(0, 0, unit.width / 2, unit.height / 2, 0, 0, Math.PI * 2);
      else if (unit.cornerRadius)
        ctx.roundRect(
          -unit.width / 2,
          -unit.height / 2,
          unit.width,
      ...
      else ctx.rect(-unit.width / 2, -unit.height / 2, unit.width, unit.height);
      ctx.fill();
    }
    ctx.restore();
  }
...
export function hitTest(frame: Frame, x: number, y: number): string | null {
  for (let i = frame.units.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      (u.kind !== "ellipse" ||
        (p.x / (u.width / 2)) ** 2 + (p.y / (u.height / 2)) ** 2 <= 1)
    )
      return u.layerId;
  }
  return null;
